...
//
// Per-run correlation (gen_ai.request.id / gen_ai.conversation.id) is set via
// the agent's `traceAttributes` per request (see agentEndpoint), not a global
// span processor, so concurrent requests never share trace state.
...
async function agentEndpoint(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    // Fresh agent per request, tagged so Agent Health can match traces to its run.
